...
def verify_factual_answer(
    ...
    the source of truth -- so refusals are attributed to the Generation/LLM
    layer rather than falsely to "Qdrant Retrieval".
    """
        ...
        if not has_mongo and not has_qdrant:
            ...
            if not qdrant_introspectable:
                return (
                    ...
                    "embedded lock). System returned a refusal -- failure at the Generation/LLM layer "
                    "(e.g., rate-limit), NOT at retrieval. Verify via the live API.",
                )
            return (
                "FAIL",
                "Qdrant Retrieval",
    ...
    if ticker and ticker != "ALL" and year:
        try:
            for d in mongo_find_by_ticker_year(ticker, year, limit=2000):
                cid = d.get("chunk_id")
    ...
    for claim in claims:
        ...
        matched = False
        if num:
            # Boundary match so "115186" does not embed inside a longer number.
            ...
            for doc in evidence_docs:
                if pat.search(_normalize_evidence_text(doc.get("raw_text", ""))):
                    ...
                    break
        if not matched:
            # Fallback: original term-based match (catches non-numeric claims).
...
async def phase_6_demo_script(ctx: AuditContext, client: httpx.AsyncClient) -> None:
    ...
    for q, ticker in demo_questions:
        res = await _run_chat_query_verified(ctx, client, q, ticker=ticker, session_id=sid)
        ctx.add(res)
